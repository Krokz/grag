...
Builds ./knowledge.lbdb describing the grag project: its modules, the design
    ...
    svc.define_schema(
        DefineSchemaRequest(
            ...
            rel_tables=[
                ...
                RelTableSpec(name="IMPLEMENTS", from_label="Module", to_label="Concept"),
                RelTableSpec(name="EXPOSES", from_label="Module", to_label="Tool"),
                RelTableSpec(name="INFORMS", from_label="Decision", to_label="Module"),
                RelTableSpec(name="APPLIES_TO", from_label="Gotcha", to_label="Module"),
            ],
        )
    )
...
def define_mentions_schema(svc) -> None:
    ...
    svc.define_schema(
        DefineSchemaRequest(
            rel_tables=[
                RelTableSpec(name="MENTIONS_MODULE", from_label="Chunk", to_label="Module"),
                RelTableSpec(name="MENTIONS_CONCEPT", from_label="Chunk", to_label="Concept"),
                RelTableSpec(name="MENTIONS_DECISION", from_label="Chunk", to_label="Decision"),
...
def upsert_entities(svc) -> None:
    modules = [
        ...
        ("core.engine", "module", "src/grag/core/engine.py",
        ...
        ("core.types", "module", "src/grag/core/types.py",
        ...
        ("core.errors", "module", "src/grag/core/errors.py",
        ...
        ("core.schema", "module", "src/grag/core/schema.py",
        ...
        ("core.mutate", "module", "src/grag/core/mutate.py",
        ...
        ("core.serialize", "module", "src/grag/core/serialize.py",
        ...
        ("config", "module", "src/grag/config.py",
         "GragConfig: db path, buffer pool size, query limits/hop caps, token budget and "
         "embedder settings, with GRAG_* environment variable overrides."),
        ("service", "module", "src/grag/service.py",
        ...
        ("api.main", "module", "src/grag/api/main.py",
         "FastAPI app: /api/{query,search,context,ingest} POST, /api/{schema,graph/sample,"
         "health} GET, mutation POSTs, plus the built React UI statics with an SPA "
         "fallback for unknown GET paths."),
        ("mcp_server", "module", "src/grag/mcp_server/server.py",
         "MCP stdio server (mcp>=2.0) exposing the 7-tool LLM contract. Tool errors return "
         "'ERROR: ... HINT: ...' content so the model can retry with corrections."),
        ("retrieval.search", "module", "src/grag/retrieval/search.py",
        ...
        ("retrieval.vectors", "module", "src/grag/retrieval/vectors.py",
        ...
        ("retrieval.polar", "module", "src/grag/retrieval/polar.py",
        ...
        ("retrieval.bench", "module", "src/grag/retrieval/bench.py",
        ...
        ("retrieval.context", "module", "src/grag/retrieval/context.py",
         "get_context: re-packs caller-chosen node ids (from a prior search) into a fresh "
         "token-budgeted context window."),
        ("ingest.loaders", "module", "src/grag/ingest/loaders.py",
         "Ingestion: markdown/JSON/CSV documents chunked with overlap (default 1200/150 "
         "chars) into Chunk nodes with _source provenance. Powers 'grag ingest <path>'."),
        ("cli", "module", "src/grag/cli.py",
         "argparse entry point with lazy subcommand imports: serve, mcp, ingest, bench."),
        ("ui", "ui", "ui/",
    ...
    concepts = [
        ("hybrid search", "Seeds come from RRF fusion of BM25 full-text search and vector "
         "candidates; the graph then expands seeds k hops so answers include connected, "
         "cited context rather than isolated chunks."),
        ("polar decomposition", "Embeddings are split into magnitude r (one float) and "
         "unit direction u (quantized). Codes only generate candidates; final scores are "
         "exact fp32 rescore plus graph rerank, so recall loss is bounded and measurable."),
        ("token-budgeted context packing", "Subgraphs serialize into compact text capped "
         "by a token budget, with [source: ...] citations on every node so LLM answers "
         "can be grounded and checked."),
        ("provenance", "Every table created via define_schema carries _source and "
         "_created_at; the mutation layer rejects caller writes to _-prefixed props."),
        ("schema introspection anchoring", "describe_schema renders tables, properties, "
        ...
        ("idempotent MERGE upserts", "upsert_nodes/upsert_edges use MERGE semantics so "
         "re-running a population pass updates in place instead of duplicating."),
        ("error hints for self-correction", "Errors are returned as 'ERROR: ... HINT: ...' "
         "so an LLM can fix its own request in the next tool call without human help."),
        ("single-writer embedded engine", "LadybugDB is embedded: one process holds the "
         "write lock per .lbdb file. Do not run 'grag serve' and 'grag mcp' against the "
         "same file concurrently."),
        ("LLM-built graphs", "define_schema + upsert tools are first-class, so 'turn these "
         "docs into a knowledge graph' is a conversation, not a pipeline project."),
    ]
    decisions = [
        ("LadybugDB as the engine", "Embedded columnar C++ graph engine, MIT licensed, "
        ...
        ("Python for the service layer", "Python 3.13 shell around the C++ engine: "
        ...
        ("Cypher as the query language", "One declarative graph query language for both "
         "the LLM and the UI console.",
         "LLMs already write Cypher fluently; inventing a new query language would burn "
         "tokens and invite hallucination."),
        ("int8 as the default vector codec", "Codec ladder fp32/int8/binary/polar, "
         "selected by GRAG_VECTOR_CODEC; int8 is the default.",
         "Bench on a 1500-doc corpus at dim 64: int8 is 4x smaller than fp32 with 0.998 "
         "recall@10. binary (32x) drops to 0.476; polar (14 bytes) sits at 0.766 and "
         "stays opt-in/experimental."),
        ("Polar coordinates for RAG vectors", "Store magnitude and quantized direction "
         "instead of raw cartesian fp32.",
    ...
    dependencies = [
        ...
        ("fastembed", "", "Optional local embeddings (ONNX, no torch) via the embed-local extra."),
    ]
    tools = [
        ("describe_schema", "mcp_tool", "Prompt-shaped schema with row counts and sample "
         "keys. Call before writing Cypher to avoid hallucinated labels."),
        ("define_schema", "mcp_tool", "Create node/rel tables; the LLM designs the graph "
         "for a domain. Records tables in the _grag_tables registry."),
        ("upsert_nodes", "mcp_tool", "Idempotent MERGE node writes with automatic "
         "_source/_created_at provenance."),
        ("upsert_edges", "mcp_tool", "Idempotent MERGE edge writes between existing nodes."),
        ("cypher_query", "mcp_tool", "Read-only Cypher with LIMIT clamping; write keywords "
         "are rejected with a hint pointing at the upsert tools."),
        ("search_knowledge", "mcp_tool", "Hybrid BM25 + vector seeds, RRF fusion, k-hop "
         "expansion, token-budgeted cited context."),
        ("get_context", "mcp_tool", "Re-pack chosen node ids from a prior search into a "
         "fresh token budget."),
        ("serve", "cli", "Run FastAPI + the graph UI on 127.0.0.1:8471 against one .lbdb file."),
        ("mcp", "cli", "Run the MCP stdio server for Cursor or any MCP client."),
        ("ingest", "cli", "Chunk and load markdown/JSON/CSV paths into Chunk nodes."),
        ("bench", "cli", "Codec benchmark: recall@k, latency, RSS per vector codec."),
    ]
    gotchas = [
        ("virtual memory exhaustion in tests", "LadybugDB reserves a huge virtual address "
        ...
        ("uppercase internal keys", "LadybugDB returns node/rel values with uppercase "
         ...
         "verified formats."),
        ("mixed-label union rows carry nulls", "A MATCH spanning several node labels "
        ...
        ("vite relative base breaks SPA fallback", "base './' made asset URLs relative, so "
        ...
        ("grid rows outnumber children", ".app defined 3 grid rows but only renders 2 "
        ...
        ("QUERY_VECTOR_INDEX parameter types", "The vector extension expects the query "
    ...
    people = [("Adam", "Software engineer; creator of grag. Prefers working directly in "
    ...
    nodes += [UpsertNode(label="Module", key=n, source=SRC,
                         properties={"kind": k, "path": p, "summary": s})
              for n, k, p, s in modules]
    nodes += [UpsertNode(label="Concept", key=n, source=SRC,
                         properties={"summary": s})
              for n, s in concepts]
    nodes += [UpsertNode(label="Decision", key=n, source=SRC,
                         properties={"summary": s, "rationale": r})
              for n, s, r in decisions]
    nodes += [UpsertNode(label="Dependency", key=n, source=SRC,
                         properties={"version": v, "purpose": p})
              for n, v, p in dependencies]
    nodes += [UpsertNode(label="Tool", key=n, source=SRC,
                         properties={"kind": k, "summary": s})
              for n, k, s in tools]
    nodes += [UpsertNode(label="Gotcha", key=n, source=SRC,
                         properties={"summary": s, "fix": f})
              for n, s, f in gotchas]
    nodes += [UpsertNode(label="Person", key=n, source=SRC,
                         properties={"role": r})
              for n, r in people]
    ...
    def dep(mod, dep_name, scope="runtime"):
        edges.append(UpsertEdge(type="DEPENDS_ON", from_label="Module", from_key=mod,
                                to_label="Dependency", to_key=dep_name,
                                properties={"scope": scope}, source=SRC))

    for m in ["core.engine", "core.schema", "core.mutate", "core.serialize",
              "retrieval.search", "retrieval.vectors", "retrieval.polar",
              "retrieval.bench", "retrieval.context", "ingest.loaders",
              "api.main", "mcp_server", "cli", "service", "core.types", "config"]:
    ...
    for m in ["service", "core.types", "config", "core.schema", "core.mutate",
              "core.serialize", "ingest.loaders", "retrieval.search",
              "retrieval.context", "cli"]:
        dep(m, "pydantic")

    def impl(mod, concept):
        edges.append(UpsertEdge(type="IMPLEMENTS", from_label="Module", from_key=mod,
                                to_label="Concept", to_key=concept, source=SRC))
    ...
    def exposes(mod, tool):
        edges.append(UpsertEdge(type="EXPOSES", from_label="Module", from_key=mod,
                                to_label="Tool", to_key=tool, source=SRC))

    for t in ["describe_schema", "define_schema", "upsert_nodes", "upsert_edges",
              "cypher_query", "search_knowledge", "get_context"]:
        exposes("mcp_server", t)
        exposes("service", t)
    for t in ["serve", "mcp", "ingest", "bench"]:
        exposes("cli", t)
    ...
    def informs(decision, mod):
        edges.append(UpsertEdge(type="INFORMS", from_label="Decision", from_key=decision,
                                to_label="Module", to_key=mod, source=SRC))
    ...
    informs("int8 as the default vector codec", "retrieval.vectors")
    informs("int8 as the default vector codec", "retrieval.bench")
    ...
    def applies(gotcha, mod):
        edges.append(UpsertEdge(type="APPLIES_TO", from_label="Gotcha", from_key=gotcha,
                                to_label="Module", to_key=mod, source=SRC))
...
def link_mentions(svc) -> None:
    """Connect README chunks to the entities they mention (Chunk -> Module/Concept/
    Decision/Tool) via the MENTIONS rel table."""
    rows = svc.cypher_query(QueryRequest(
        cypher="MATCH (c:Chunk) RETURN c.id AS id, c.text AS text", limit=500)).rows
    patterns: list[tuple[str, str, re.Pattern]] = []

    def add(label, key, *aliases):
        pats = [re.escape(key)] + [re.escape(a) for a in aliases]
        patterns.append((label, key,
                         re.compile(r"(?i)\b(" + "|".join(pats) + r")\b")))

    for key, path in [("core.engine", "engine.py"), ("core.types", "types.py"),
                      ("core.mutate", "mutate"), ("core.schema", "schema introspection"),
                      ("core.serialize", "serialize"), ("service", "GragService"),
                      ("api.main", "FastAPI"), ("mcp_server", "mcp_server"),
                      ("retrieval.search", "retrieval"), ("retrieval.vectors", "vectors"),
                      ("retrieval.polar", "polar.py"), ("retrieval.bench", "bench"),
                      ("ingest.loaders", "loaders"), ("ui", "graph explorer"), ("cli", "cli")]:
        add("Module", key, path)
    for key, *aliases in [
            ("hybrid search", "hybrid"), ("polar decomposition", "polar"),
    ...
    for key in ["LadybugDB as the engine", "Python for the service layer",
                "Cypher as the query language", "int8 as the default vector codec",
                "Polar coordinates for RAG vectors", "One process for API and UI",
                "Errors carry hints"]:
        add("Decision", key)
    for key in ["describe_schema", "define_schema", "upsert_nodes", "upsert_edges",
                "cypher_query", "search_knowledge", "get_context", "serve", "mcp",
                "ingest", "bench"]:
        add("Tool", key)

    rel_type = {"Module": "MENTIONS_MODULE", "Concept": "MENTIONS_CONCEPT",
                "Decision": "MENTIONS_DECISION", "Tool": "MENTIONS_TOOL"}
    edges: list[UpsertEdge] = []
    for chunk_id, text in rows:
        for label, key, pat in patterns:
            if pat.search(text or ""):
                edges.append(UpsertEdge(type=rel_type[label], from_label="Chunk",
                                        from_key=chunk_id, to_label=label, to_key=key,
                                        source=SRC))
...
def verify(svc) -> None:
    ...
    print("\n=== sanity searches ===")
    for q in ["why is the backend python", "which vector codec should I use",
              "mmap failure", "what tools does the MCP server expose"]:
